<?php
/**
 * Plugin Name:       The Events Calendar Extension: shortcode-desktop-view
 * Plugin URI:        
 ...
 */
define( 'TRIBE_EXTENSION_SHORTCODE_DESKTOP_VIEW_FILE', __FILE__ );
 ...
 * @since 1.0.0
 */
function tribe_extension_shortcode_desktop_view() {
	...
	Tribe__Autoloader::instance()->register_prefix(
		'\\Tribe\\Extensions\\Shortcodedesktopview\\',
		__DIR__ . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Tribe',
		'shortcode-desktop-view'
	);

	// Deactivates the plugin in case of the main class didn't autoload.
	if ( ! class_exists( '\Tribe\Extensions\Shortcodedesktopview\Plugin' ) ) {
		tribe_transient_notice(
			'shortcode-desktop-view',
			'<p>' . esc_html__( 'Couldn\'t properly load "The Events Calendar Extension: shortcode-desktop-view" the extension was deactivated.', '__TRIBE_DOMAIN__' ) . '</p>',
			[],
	...
	tribe_register_provider( '\Tribe\Extensions\Shortcodedesktopview\Plugin' );
}

// Loads after common is already properly loaded.
add_action( 'tribe_common_loaded', 'tribe_extension_shortcode_desktop_view' );
