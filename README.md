...
* `shortcode-desktop-view` - Plugin Human-readable name
* `Shortcodedesktopview` - Which namespace we will use for the plugin
* `shortcode-desktop-view` - Uses using dashes normally
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix
* `shortcode_desktop_view` - Uses underscores, so it's safe for variables
* `SHORTCODE_DESKTOP_VIEW` - Uppercase of clean slug
* `` - By default empty
